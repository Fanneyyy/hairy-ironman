...
ostream& operator <<(ostream& outs, const Person& p) {
    outs << fixed;
    outs << left << setw(30) << p.name << setw(15) << p.gender << setw(15) << left << p.yearOfBirth;
...
string Person::toString() {
    return name + " " + gender + " " + yearOfBirth + " " + yearOfDeath;
}
...
string Person::stringToLower(string original) {
    string result = "";
